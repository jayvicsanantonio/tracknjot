...
export function NoteContentEditor({ note, onUpdateNote }: NoteContentEditorProps) {
  ...
  const handleAutoSave = (field: 'title' | 'content', value: string) => {
    if (!note) return;
  ...
  if (!note) {
    return (
      <div className="flex-1 bg-[rgb(32,32,32)] dark:bg-[rgb(32,32,32)] bg-white h-screen overflow-hidden flex flex-col">
        <div className="flex items-center justify-center h-full">
          <p className="text-base text-[hsl(var(--muted-foreground))]">Select a note to view</p>
        </div>
      </div>
    )
  }
